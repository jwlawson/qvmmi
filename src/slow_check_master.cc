...
void
SlowCheckMaster::handle_result(int result, int worker) {
...
SlowCheckMaster::MMITracker&
SlowCheckMaster::get_tracker(const MatrixPtr& matrix) {
...
void
SlowCheckMaster::handle_tracker(const MMITracker& tracker) {
  bool mmi = true;
  if (tracker.infinite) {
    mmi = false;
  }
