...
FastMaster<T, M>::FastMaster() : Master(), map_() {}
...
template <class T, class M>
void
FastMaster<T, M>::handle_result(int result, int worker) {
  if (result == 1) {
    /* Matrix is mmi. */
...
template class FastMaster<cluster::StreamExtIterator<cluster::QuiverMatrix>,
